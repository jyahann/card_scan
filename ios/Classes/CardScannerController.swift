...
public class CardScannerController: VisionController {
    ...
    init(
        ...
        self.numberTracker = StringTracker(
            threshold: configuration.cardNumberThreshold
        )
        self.expDateTracker = StringTracker(
            threshold: configuration.cardExpiryThreshold
        )
        self.fullNameTracker = StringTracker(
            threshold: configuration.cardHolderThreshold
        )
    ...
    public override func observationsHandler(
        ...
        if observationsCount == 20 && (foundNumber == nil)
            && cameraBrightness < 0
        {
            // toggleTorch(on: true)
        }
        ...
        for observation in observations {
            ...
            guard
                let candidate = observation.topCandidates(maximumCandidates)
                    .first
            else { continue }
            print("[Text recognition] ", candidate.string)

            if foundNumber == nil,
                let cardNumber = candidate.string.checkCardNumber()
            {
    ...
    private func searchCardHolder(_ text: String) {
        ...
        } else if let fullName = text.checkFullName(
            firstName: firstNameSuggestion,
            lastName: lastNameSuggestion
        ) {
            trackFullName((fullName, nil))
        }
    }
    ...
    private func shouldStopScanner() {
        let number = foundNumber ?? numberTracker.getConsensusString()
        let expDate = foundExpDate ?? expDateTracker.getConsensusString()
        let holder = foundCardHolder ?? fullNameTracker.getConsensusString()

        // Если вообще ничего не нашли — выходим
        if (number == nil || number?.isEmpty == true)
            && (expDate == nil || expDate?.isEmpty == true)
            && (holder == nil || holder?.isEmpty == true)
        {
            return
        }
        ...
        if foundNumber != nil
            && ((foundExpDate != nil && foundCardHolder != nil)
                || (observationsCount
                    >= self.configuration.observationsCountLimit))
        {
